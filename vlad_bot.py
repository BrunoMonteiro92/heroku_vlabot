...
def handle(msg):
    ...
    if (command.startswith('/')):
        ...
        response = ["Gua", "Beisso aí", "Risos", "Hausha7hausua7e", "Calma", "Te fode carlos", "Nossa", "Você tá bem, cara?", "Uia", "Te fode carlos", ".", "Egua", "Nao ma", "Bacana", "Essa fera", "Que loucura", "Te fode carlos", "O loco Bichow", "Tenso", "Diz",
                    "Rs", "E foi indo e foi indo... E tamo aqui agora", "Meu nome é Vladimir Lima tenho 24 anos, faço cerveja, ando de bicicleta e só", "Complicado esse humor", "boa", "Esse tempo todo?", "Vai fumar maconha ma\nQue tu fica de boas\nMenos exaltado", "Bebam água"]
        ...
        if command == '/mito' or command == '/mito@vlademeeer_bot':
            bot.sendMessage(
                chat_id, 'Opa querido(a), esse comando foi substituído pelo /fera, por motivos de **FASCISMO**, perdão pelo incoveniente', parse_mode='Markdown')

        if command == '/askvlad' or command == '/askvlad@vlademeeer_bot':
            print('Command ' + command +
                  ' received from chat ' + repr(chat_id) + ' ...')
            bot.sendMessage(
                chat_id, "Algo de errado não está certo, cadê a pergunta?", reply_to_message_id=msg_id)
